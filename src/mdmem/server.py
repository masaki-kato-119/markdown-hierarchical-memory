...
from .store import MemoryFile, get_root, iter_content_files, load_all
...
def _mf_to_dict(mf: MemoryFile, root: Path, include_body: bool = True) -> dict[str, Any]:
    d = {
        "id": mf.id,
...
def get_server_info() -> dict:
    ...
    MDMEM_ROOT env var or the "memory" (cwd-relative) default, and a content
    file count. Call this first when get_index/list_memory_files and
    read_memory_file seem to disagree -- that usually means the caller is
    talking to a server resolving a different root than expected, not a
    caching problem."""
    root = get_root()
    return {
        "root": str(root),
...
def update_metadata(
    ...
    mf = manager.update_metadata(
        ...
        expected_updated=expected_updated, actor=actor, rationale=rationale,
    )
    return _mf_to_dict(mf, root)
...
def split_memory_file(
    ...
    split action). Either name an existing `section_to_extract` heading (it is
    removed from the source and used as the new file's content), or supply
    `extracted_content` directly and leave the source body untouched. Wires up
    the bidirectional link (§12) and index.md entry automatically."""
    root = get_root()
    ...
    return {"source": _mf_to_dict(source_mf, root), "new": _mf_to_dict(new_mf, root)}
...
def move_to_archive(
    ...
    root = get_root()
    mf = manager.move_to_archive(root, id, expected_updated=expected_updated, actor=actor, rationale=rationale)
    return _mf_to_dict(mf, root)


@mcp.tool()
def check_archive_candidates() -> list[dict]:
    """Report files matching spec §13's forget condition (access_count < 3 AND
    last_access > 180 days AND not pinned). This only reports candidates --
    ...
    root = get_root()
    return manager.check_archive_candidates(root)
